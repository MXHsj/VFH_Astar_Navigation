function OptimalPath=ASTARPATH2(StartX,StartY,MAP,RegisteredGoal,Connecting_Distance)
% function OptimalPath=ASTARPATH2(StartY,StartX,MAP,GoalRegister,Connecting_Distance)
%Version 1.1
...
% RegisteredGoal=[row col];
% Nodesfound=size(RegisteredGoal,1);
Nodesfound=1;

for k=1:Height
    for j=1:Width
        if MAP(k,j)==0
            Mat=RegisteredGoal-(repmat([j k],(Nodesfound),1));
